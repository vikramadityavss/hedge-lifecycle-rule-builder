...
export class RuleBuilderComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.authService.isAuthenticated().subscribe(authenticated => {
      ...
      // If authenticated, continue with initialization
  ...
  private initializeComponent(): void {
    ...
    this.lifecycleStageService.getSelectedStageId().subscribe(stageId => {
      ...
      if (stageId) {
        this.loadRulesForStage(stageId);
      } else {
  ...
  private loadRulesForStage(stageId: string): void {
    this.isLoading = true;
    this.supabaseService.getRulesByStageId(stageId).subscribe(
      rules => {
        this.rules = rules;
        this.isLoading = false;
      },
      error => {
        console.error('Error loading rules for stage:', error);
        this.messageService.add({
          severity: 'error',
          summary: 'Error',
          detail: 'Failed to load rules for this stage.'
        });
        this.isLoading = false;
      }
    );
  ...
  private loadFieldDefinitions(): void {
    this.isLoading = true;
    this.ruleService.getFieldDefinitions().subscribe(
      fields => {
          ...
          if (success) {
            ...
            this.ruleService.deleteRule(rule.id).subscribe(deleteSuccess => {
              if (deleteSuccess) {
                ...
                this.loadRulesForStage(this.currentStageId!);
              } else {
                this.messageService.add({
                  severity: 'error',
                  summary: 'Error',
            ...
            this.messageService.add({
              severity: 'error',
              summary: 'Error',
              detail: 'Failed to remove rule from stage'
            });
            this.isLoading = false;
          }
        });
  ...
  saveRule(): void {
    ...
    // Add user IDs from Supabase
    const userId = this.supabaseService.getCurrentUserId();
    if (userId) {
      if (this.isNewRule) {
        this.currentRule.createdBy = userId;
      }
      this.currentRule.updatedBy = userId;
    }
        ...
        this.lifecycleStageService.addRuleToStage(this.currentStageId!, rule).subscribe(success => {
          ...
          if (success) {
            ...
            this.messageService.add({
              severity: 'error',
              summary: 'Error',
              detail: 'Failed to add rule to stage'
            });
          }
        });
      });
  ...
  resetNewCondition(): void {
    this.newCondition = {
      id: this.ruleService.generateId(),
      field: '',
